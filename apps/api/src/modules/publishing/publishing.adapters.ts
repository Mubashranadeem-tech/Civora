...
export class LinkedInAdapter implements PublishingAdapter {
  ...
  constructor(private readonly config: ConfigService) {
    this.accessToken = config.get<string>('LINKEDIN_ACCESS_TOKEN');
    this.orgId = config.get<string>('LINKEDIN_ORGANIZATION_ID');
  }
...
export class TwitterAdapter implements PublishingAdapter {
  ...
  constructor(private readonly config: ConfigService) {
    const apiKey = config.get<string>('TWITTER_API_KEY');
    const apiSecret = config.get<string>('TWITTER_API_SECRET');
    const accessToken = config.get<string>('TWITTER_ACCESS_TOKEN');
    const accessSecret = config.get<string>('TWITTER_ACCESS_TOKEN_SECRET');
...
export class WordPressAdapter implements PublishingAdapter {
  ...
  constructor(private readonly config: ConfigService) {
    this.wpUrl = config.get<string>('WORDPRESS_URL');
    this.wpUser = config.get<string>('WORDPRESS_USERNAME');
    this.wpPass = config.get<string>('WORDPRESS_APP_PASSWORD');
  }

  isConfigured(): boolean {
    return !!(this.wpUrl && this.wpUser && this.wpPass);
  }
...
export class WebhookAdapter implements PublishingAdapter {
  ...
  async publish(content: string): Promise<{ url: string }> {
    ...
    if (this.webhookSecret) {
      headers['X-Civora-Secret'] = this.webhookSecret;
    }

    const response = await fetch(this.webhookUrl!, {
      method: 'POST',
      headers,
      body: JSON.stringify({ content, source: 'civora' }),
    });
